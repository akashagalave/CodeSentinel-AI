...
cost_histogram = Histogram(
    "llm_cost_usd", "LLM API cost per call USD", ["service"],
...
def compress_if_needed(text: str, max_tokens: int = 4000, model: str = "gpt-4o") -> str:
    """Simple line-by-line truncation — no heavy ML model needed."""
    current = count_tokens(text, model)
    if current <= max_tokens:
        return text
    logger.info(f"Truncating: {current} → {max_tokens} tokens")
    lines = text.split("\n")
